	...
	self.processOption = function(data){
		...
		if(t=="file"){
			...
			if(data.source == "import" || ~data.source.indexOf("filedrop")){ //data source: import
				...
				self[trackname] = ko.computed(function(){ var item = importedFiles.get(self[trackname].filename()); return item?~item.type.indexOf('seq')?"sequence":~item.type.indexOf('tree')?"tree":"OK":""; });
...
var uimodel = new interfaceModel({"title": "Min. ORF length", "option": "min-orf-length", "desc": "minimum ORF length", "type": "int", "default": 100});
